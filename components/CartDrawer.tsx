...
export const CartDrawer: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex justify-end">
      <div className="absolute inset-0 bg-slate-900/50 backdrop-blur-sm transition-opacity" onClick={toggleCart} />
      <div className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          {items.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-slate-400 space-y-4">
               ...
               <button onClick={toggleCart} className="text-brand-600 font-medium hover:underline">Browse Gallery</button>
            </div>
          ) : (
            items.map((item) => (
        ...
        {items.length > 0 && (
          <div className="p-6 border-t border-slate-100 bg-slate-50">
            ...
            <button onClick={handleCheckout} disabled={isCheckingOut} className="w-full py-4 bg-slate-900 text-white rounded-xl font-bold hover:bg-slate-800 transition-all shadow-lg flex items-center justify-center gap-2 disabled:opacity-70">
              {isCheckingOut ? <Loader2 className="animate-spin" /> : <Lock className="w-4 h-4" />}
              {isCheckingOut ? 'Processing...' : 'Secure Checkout'}
            </button>
            <p className="text-center text-xs text-slate-400 mt-4">Payments processed securely via Stripe.</p>
          </div>
        )}
      </div>
    </div>
